...
- **Retrieval and generation are separate steps.** A cheap/fast method (a local
  ...
  see `app/retrieval.py` — briefly swapped for the lighter `fastembed`/ONNX
  Runtime to fit a 512MB free-tier host, reverted once hosting moved to a
  paid tier with real RAM, since sentence-transformers is the more mature,
  broadly-supported library and that was the only reason to leave it)
  narrows candidate pairs, both jurisdiction and cross-discipline, before
  the LLM reasons over them — this keeps the LLM from
